...
class LettuceCats3RateLimiterSpec extends LettuceRateLimiterSpec[IO] with Cats3BaseSpec {
  override def rateLimiter(strategy: Strategy): RateLimiter[IO] =
    LettuceCats3RateLimiter.useClient[IO](redisClient, strategy).unsafeRunSync()
