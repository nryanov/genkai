...
class RedissonCats3RateLimiterSpec extends RedissonRateLimiterSpec[IO] with Cats3BaseSpec {
  override def rateLimiter(strategy: Strategy): RateLimiter[IO] =
    RedissonCats3RateLimiter.useClient[IO](redisClient, strategy).unsafeRunSync()
